...
FROM Shift
WHERE Shift_Date >= DATEADD(WEEK, -1, GETDATE()) AND Shift_Date < GETDATE();

--List of which employees worked breaker shifts in the last month.
SELECT DISTINCT EMP_Working
FROM Shift
WHERE Shift_Type = 'Breaker' AND Shift_Date >= DATEADD(MONTH, -1, GETDATE()) AND Shift_Date < GETDATE();

--How many breaker shifts are scheduled this week?
SELECT COUNT(*) AS Breaker_Shifts_Scheduled
FROM Shift
WHERE Shift_Type = 'Breaker' AND Shift_Date >= GETDATE() AND Shift_Date < DATEADD(WEEK, 1, GETDATE());
...
WHERE EMP_TYPE_ID = (SELECT Type_ID FROM Employee_Type WHERE Type_Title = 'Slot Attendant') AND Shift_Date = CONVERT(DATE, GETDATE());
...
WHERE EMP_TYPE_ID = (SELECT Type_ID FROM Employee_Type WHERE Type_Title = 'Slot Attendant') AND EMP_ID NOT IN (
    ...
    WHERE Section_Description = 'NORTH' AND Shift_Date >= DATEADD(MONTH, -1, GETDATE()) AND Shift_Date < GETDATE()
);
...
FROM Warning
WHERE Warn_Issued_To = employee_id AND Warn_Level = 1;
...
SELECT EMP_ID, EMP_Name, Cert_Type, Cert_Exp_Date
FROM Employee
...
SELECT EMP_ID, EMP_Name, Training_Type, Training_Exp_Date
FROM Employee
JOIN Training ON Employee.EMP_ID = Training.EMP_Trained
WHERE Training_Date < GETDATE();

-- List of employees who have expired training
-- (Assuming a training table and considering training expiration)
SELECT EMP_ID, EMP_Name, Traning_Type, Training_Exp_Date
FROM Employee
JOIN Training ON Employee.EMP_ID = Training.EMP_Trained
WHERE Training_Exp_Date < GETDATE();

-- How many uniforms remain un-allocated?
SELECT Inv_Quantity - COUNT(Uni_ID) AS UnallocatedUniforms
FROM Inventory
LEFT JOIN Uniform ON Inventory.Inv_ID = Uniform.Uni_ID
